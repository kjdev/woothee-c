...
libwoothee.so: libwoothee.so.1.2.0
	ln -f -s $^ $@

libwoothee.so.1.2.0: $(OBJS)
...
install:
	...
	install -m755 libwoothee.so.1.2.0 $(DESTDIR)$(LIBDIR)
	ln -f -s libwoothee.so.1.2.0 $(DESTDIR)$(LIBDIR)/libwoothee.so.1
	ln -f -s libwoothee.so.1.2.0 $(DESTDIR)$(LIBDIR)/libwoothee.so
